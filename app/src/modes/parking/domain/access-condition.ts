...
import { parseConditionalTag, parseOpeningHours } from '@osm-editor-kit/osm-tag-syntax'
...
export function getConditions(tags: OsmTags, side?: string) {
  ...
  if (maxstayConditionalValue) {
    conditions.conditionalValues?.push(
      ...parseConditionalTag(maxstayConditionalValue).map((x) => ({
  ...
  if (accessConditionalValue) {
    conditions.conditionalValues?.push(
      ...parseConditionalTag(accessConditionalValue).map((x) => ({
        condition: parseOpeningHours(x.condition),
  ...
  if (restrictionConditionalValue) {
    conditions.conditionalValues?.push(
      ...parseConditionalTag(restrictionConditionalValue).map((x) => ({
        condition: parseOpeningHours(x.condition),
  ...
  if (maxstayValue && maxstayValue !== 'no') conditions.default = 'disc'

  if (feeValue === 'yes') conditions.default = 'ticket'

  if (laneValue && ['no', 'separate'].includes(laneValue)) conditions.default = laneValue

  if (restrictionValue) conditions.default = restrictionValue

  if (accessValue) conditions.default = mapAccessValue(tags, accessValue, side)
