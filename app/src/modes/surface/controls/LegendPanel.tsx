...
const legendMetaById = Object.fromEntries(
  surfaceLegendItems.map((item) => [
    ...
    {
      color: item.color,
      dotted: 'dotted' in item ? item.dotted : false,
      missing: 'missing' in item ? item.missing : false,
    },
  ]),
) as Record<
  (typeof surfaceLegendItems)[number]['id'],
  { color: string; dotted: boolean; missing: boolean }
>
  ...
  return (
    <div className="flex flex-col gap-1">
      {getSurfaceLegendItems().map((item) => {
        ...
        if (meta.missing) {
          return <MissingDataLegendLine key={item.id}>{item.label}</MissingDataLegendLine>
        }
        return (
