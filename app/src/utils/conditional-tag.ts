export function parseConditionalTag(tag: string) {
  ...
  if (
    prevConditionEndPosition == null ||
    prevConditionEndPosition < tag.length - (tag.endsWith(')') ? 2 : 1)
  ) {
    const startPosition = prevConditionEndPosition ? prevConditionEndPosition + 1 : 0
    parsedConditionalTag.push(
      parseConditionalValue(tag.substring(startPosition, tag.length - (tag.endsWith(')') ? 1 : 0))),
    )
  }
