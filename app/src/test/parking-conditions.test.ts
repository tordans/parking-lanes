...
describe('#getConditions()', () => {
  ...
  test('fee={time_interval}', async () => {
    ...
    const receivedConditions = getConditions(tags)
    expect(receivedConditions.default).toBe('free')
    expect(receivedConditions.conditionalValues).toHaveLength(1)
    expect(receivedConditions.conditionalValues[0]?.parkingCondition).toBe('ticket')
    expect(receivedConditions.conditionalValues[0]?.condition).not.toBeNull()
  })
  ...
  test('fee:conditional=no @ ({time_interval})', async () => {
    ...
    const receivedConditions = getConditions(tags)
    expect(receivedConditions.default).toBe('ticket')
    expect(receivedConditions.conditionalValues).toHaveLength(1)
    expect(receivedConditions.conditionalValues[0]?.parkingCondition).toBe('free')
    expect(receivedConditions.conditionalValues[0]?.condition).not.toBeNull()
  })
  ...
  test('maxstay:conditional and fee:conditional', async () => {
    ...
    const receivedConditions = getConditions(tags, 'right')
    expect(receivedConditions.default).toBe('free')
    expect(receivedConditions.conditionalValues).toHaveLength(2)
    expect(receivedConditions.conditionalValues[0]?.parkingCondition).toBe('ticket')
    expect(receivedConditions.conditionalValues[1]?.parkingCondition).toBe('disc')
    expect(receivedConditions.conditionalValues[0]?.condition).not.toBeNull()
    expect(receivedConditions.conditionalValues[1]?.condition).not.toBeNull()
  })
  ...
  test('loading_only', async () => {
    ...
    const receivedConditions = getConditions(tags, 'right')
    expect(receivedConditions.default).toBe('no_parking')
    expect(receivedConditions.conditionalValues).toHaveLength(1)
    expect(receivedConditions.conditionalValues[0]?.parkingCondition).toBe('loading_only')
    expect(receivedConditions.conditionalValues[0]?.condition).not.toBeNull()
  })
