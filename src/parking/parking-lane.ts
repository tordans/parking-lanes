...
export function parseChangedParkingLane(
  newOsm: OsmWay,
  lanes: ParkingLanes,
  datetime: Date,
  zoom: number,
  ...
  for (const side of ['right', 'left'] as Side[]) {
    ...
    if (conditions.default != null) {
      if (lanes[id]) {
        ...
        const leafletPolyline = createPolyline(
          polyline,
          conditions,
          side,
          newOsm,
          isMajor ? (laneStyle[zoom].offsetMajor ?? 1) : (laneStyle[zoom].offsetMinor ?? 0.5),
          isMajor,
          zoom,
        )
        lanes[laneId] = leafletPolyline
...
function generateLaneId(osm: OsmWay, side?: 'left' | 'right', conditions?: ParkingConditions) {
  if (!conditions) return 'empty' + osm.id

  return side! + osm.id
}

function createPolyline(
  line: L.LatLngLiteral[],
...
export function updateLaneStylesByZoom(lanes: ParkingLanes, zoom: number): void {
  for (const lane in lanes) {
    if (lane === 'right' || lane === 'left' || lane.startsWith('empty')) continue
...
export function getBacklights(
  polyline: L.LatLngExpression[],
  zoom: number,
): { right: L.Polyline; left: L.Polyline } {
  const n = 3
