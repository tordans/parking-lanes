...
import {
  parseParkingPoint,
  updatePointColorsByDate,
  updatePointStylesByZoom,
} from './parking-point'
...
export function initMap() {
  ...
  if (!document.location.href.includes('#')) {
    ...
    map.setView(cookieLocation?.location ?? new L.LatLng(51.591, 24.609), cookieLocation?.zoom ?? 5)
  }
  ...
  Panel(
    async () => await downloadParkingLanes(map),
    handleCutLaneClick,
    handleOsmChange,
...
async function downloadParkingLanes(map: L.Map): Promise<void> {
  ...
  for (const relation of Object.values(newData.relations)) {
    if (relation.tags?.amenity === 'parking') {
      if (areas[relation.type + relation.id]) continue

      const newAreas = parseParkingRelation(
        relation,
        newData.nodeCoords,
        newData.ways,
        map.getZoom(),
      ...
      if (newAreas !== undefined) addNewAreas(newAreas, map)
    }
  }

  for (const way of Object.values(newData.ways)) {
    if (way.tags?.highway) {
      if (lanes['right' + way.id] || lanes['left' + way.id] || lanes['empty' + way.id]) continue

      const newLanes = parseParkingLane(way, newData.nodeCoords, map.getZoom(), editorMode)
      if (newLanes !== undefined) addNewLanes(newLanes, map)
    } else if (way.tags?.amenity === 'parking') {
      if (areas[way.type + way.id]) continue
  ...
  for (const node of Object.values(newData.nodes)) {
    if (node.tags?.amenity === 'parking_entrance' || node.tags?.amenity === 'parking') {
      if (points[node.id]) continue

      const newPoints = parseParkingPoint(node, map.getZoom(), editorMode)
      if (newPoints !== undefined) addNewPoint(newPoints, map)
    }
  }
}
...
async function handleEditorChange(state: AppStateStore, prevState: AppStateStore) {
  if (state.editorMode === prevState.editorMode) return
...
function handleCutLaneClick(osm: OsmWay) {
  ...
  for (const nd of osm.nodes.slice(1, osm.nodes.length - 1)) {
    markers[nd] = L.marker(osmData.nodeCoords[nd], {
      icon: cutIcon,
      // @ts-expect-error
      ndId: nd,
      wayId: osm.id,
