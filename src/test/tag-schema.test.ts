...
describe('tag schema', () => {
    ...
    test('shouldShowTag shows orientation for lane positions', () => {
        const orientationTag = parkingLaneTags.find(tag => tag.template === 'parking:{side}:orientation')!
    ...
    test('getDependentTagKeys resolves dependent tag templates', () => {
        ...
        expect(getDependentTagKeys(feeTag, 'right')).toStrictEqual([
    ...
    test('restriction reason only shows when restriction is set', () => {
        ...
        const restrictionReasonTag = parkingLaneTags.find(tag => tag.template === 'parking:{side}:restriction:reason')!
