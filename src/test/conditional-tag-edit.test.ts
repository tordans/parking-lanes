...
describe('conditional tag edit', () => {
    ...
    test('parseConditionalTagForEdit appends empty editable slot', () => {
        ...
        expect(parseConditionalTagForEdit(undefined)).toStrictEqual([
            { value: '', condition: null },
        ])
    })
    ...
    test('buildConditionalTagValue rebuilds tag from edited part', () => {
        ...
        expect(buildConditionalTagValue(parsed, { value: 'disc', condition: 'Sa 08:00-15:00' }, 1))
            .toBe('free @ (Mo-Fr 09:00-19:00); disc @ (Sa 08:00-15:00)')

        expect(buildConditionalTagValue(parsed, { value: '', condition: 'Mo' }, 0))
            .toBe('ticket @ (19:00-20:00)')
    })
})
